...
void draw() {
  ...
  //
  float[][] scaleWithMouse = scaleMatrix(scaleSize, scaleSize, scaleSize); 
  float[][] rotateWithKeyX= rotateXMatrix(rotateSizeX); 
  float[][] rotateWithKeyY= rotateYMatrix(rotateSizeY); 
  //
  My3DPoint eye = new My3DPoint(0, 0, -5000); 
  My3DPoint origin = new My3DPoint(0, 0, 0);
  My3DBox input3DBox = new My3DBox(origin, 100, 150, 300);
  //Scale with the mouse
  input3DBox = transformBox(input3DBox, scaleWithMouse);
  input3DBox = transformBox(input3DBox, rotateWithKeyX);
  input3DBox = transformBox(input3DBox, rotateWithKeyY);
...
float[][] rotateXMatrix(float angle) { 
  return(new float[][] {{1, 0, 0, 0}, 
    {0, cos(angle), -sin(angle), 0}, {0, sin(angle), cos(angle), 0}, {0, 0, 0, 1}});
}
