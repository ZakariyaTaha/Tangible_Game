...
OpenCV opencv;
class Thresholding extends PApplet {
  Capture cam;
  PVector getRotation;
  ...
  void draw() {
    ...
    TwoDThreeD twoD = new TwoDThreeD(img.width, img.height, frameRate);
    for (int i = 0; i < quadList.size(); i++) {
      quadList.get(i).add(0, 0, 1);
    }
    image(imgT, 2*width/3, 0);
    getRotation = twoD.get3DRotations(quadList);
  }
  ...
  PVector getRotation(PImage im) {
    ...
    imgT = convolute(imgT, GaussianKernel, GaussianFactor);  
    imgT = Blob.findConnectedComponents(imgT, false);
    TwoDThreeD twoD = new TwoDThreeD(img2.width, img2.height, frameRate);
    for (int i = 0; i < quadList.size(); i++) {
      quadList.get(i).add(0, 0, 1);
    }
    //image(imgT, 2*width/3, 0);
    PVector twoToThree = twoD.get3DRotations(quadList);
    twoToThree.x = restrict(twoToThree.x);
    twoToThree.y = restrict(twoToThree.y);
    twoToThree.z = restrict(twoToThree.z);
    return twoToThree;
  }
